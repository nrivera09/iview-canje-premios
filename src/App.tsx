    ...
    <>
      ...
      {showBug && (
        <button
          ...
          className="z-50 font-bold text-xl bg-red-600 hover:bg-red-900 p-1 rounded-md overflow-hidden   mx-auto transition-all cursor-pointer hover:shadow-xl h-[30px] sm:h-[30px] w-[30px] sm:w-[30px] items-center justify-center absolute top-0 right-0 mt-[50px] mr-1 text-white "
        >
          X
        </button>
      )}
      <div className="absolute top-1/2 right-0 mr-1 mt-[-50px] flex flex-col items-end gap-2">
        <button
          onClick={() => {
            playSound("button");
            setShowBug(!showBug);
        ...
        >
          <img src={slotMachineBug} alt="" className="w-[30px]" />
        </button>
      </div>
      {isDevEnv && (
